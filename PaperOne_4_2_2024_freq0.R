...
TripleSEIIIRiso= function(CM, tauL, epsilonL, thetaL, delta, gammaL, piL, tauG,thetaG,omegaG, chiG, gammaG, piG, epsilonIS, gammaIS, epsilonIAL, gammaIAL, gammaIAG, iota, psi, zeta, sigma,mu, nu, isolationdelay) { {
  ...
  LWEVINF= as.data.frame(matrix(rep(0,N), nrow=N, ncol = 1)) # Single column matrix for lab workers who have ever been infected (starts at 0)
...
TripleSEIIIRiso2= function(CM, tauL, epsilonL, thetaL, delta, gammaL, piL, tauG,thetaG,omegaG, chiG, gammaG, piG, epsilonIS, gammaIS, epsilonIAL, gammaIAL, gammaIAG, iota, psi, zeta, sigma,mu, nu, isolationdelay) { {
  ...
  while(t<100){ #as long as t<100, and...
    ...
    LWEVINF= cbind(LWEVINF, nextLWEVINF) #bind LWEVINF, nextLWEVINF together
    GPEVINF= cbind(GPEVINF, nextGPEVINF) #bind GPEVINF, nextGPEVINF together
    Incidence= cbind(Incidence, nextstepcases) #bind Incidence, nextstepcases together
    R= cbind(R, nextR) #bind R, nextR matrices together
  }
}
  freq= rep(freq, t)
  ...
  res=list(SL=SL, SG=SG, ENL=ENL, ENG=ENG, ENISO=ENISO, IPL=IPL, IPG=IPG, IPISO=IPISO, IAL=IAL, IAG=IAG, ISL=ISL, ISG=ISG, ISISO=ISISO, EVISO=EVISO,LWEVINF= LWEVINF,GPEVINF=GPEVINF, R=R, freq=freq, sigma=sigma, isolationdelay=isolationdelay, Incidence=Incidence) #save results in a list of matrices SL,SG,ENL,ENG,ENISO, IPL,IPG, IPISO, IAL, IAG, ISL,ISG, ISISO, R
...
sim7=TripleSEIIIRiso(cm6, tauL=0.5, epsilonL=0.5, thetaL=1, delta=0.5, gammaL=0.25, piL=0.1, tauG=0.05, thetaG=0,omegaG=0, chiG=0.01, gammaG=0.25,piG=0, epsilonIS=0.5, gammaIS=0.25, epsilonIAL = 0.5, gammaIAL=0.25, gammaIAG=0.25, iota=0.2, psi=0.8, zeta=0.9, sigma=0.95,mu=0.15, nu=0.5, isolationdelay = 0)
#sim7
...
sim8=TripleSEIIIRiso2(cm6, tauL=0.5, epsilonL=0.5, thetaL=1, delta=0.5, gammaL=0.25, piL=0.1, tauG=0.05, thetaG=0, omegaG=0, chiG=0.01, gammaG=0.25,piG=0, epsilonIS=0.5, gammaIS=0.25, epsilonIAL = 0.5, gammaIAL=0.25, gammaIAG=0.25, iota=0.2, psi=0.8, zeta=0.9, sigma=0.95,mu=0.15, nu=0.5, isolationdelay = 0)
sim8
end_time <- Sys.time()
end_time - start_time
...
par_tab <- expand.grid(thetaL= 0, sigma= seq(0.5,1,0.1), 
...
####Run Sims####
start_time <- Sys.time()
total_sims = nrow(par_tab) * n_sims # Total number of simulations
output <- vector("list", total_sims) # Pre-allocate list to hold all output
unique_sim_counter <- 1 # Counter for unique simulations
...
for(i in 1:nrow(par_tab)){ # For every row in parameter table representing a unique parameter combination [outer loop]
  ...
  for(j in 1:n_sims){ # For every simulation... [inner loop]
    ...
    unique_seed = (i - 1) * n_sims + j # Calculate a unique seed for this simulation
    set.seed(unique_seed) # Set the unique simulation seed
    
    # Perform the simulation and store the output
    output[[unique_sim_counter]] <- TripleSEIIIRiso2(CM= cm6, thetaL =par_tab$thetaL[i], sigma= par_tab$sigma[i],isolationdelay = par_tab$isolationdelay[i], par_tab$tauL[i], par_tab$epsilonL[i], par_tab$delta[i], par_tab$gammaL[i], par_tab$piL[i], par_tab$tauG[i], par_tab$thetaG[i], par_tab$omegaG[i], par_tab$chiG[i], par_tab$gammaG[i], par_tab$piG[i], par_tab$epsilonIS[i], par_tab$gammaIS[i], par_tab$epsilonIAL[i], par_tab$gammaIAL[i], par_tab$gammaIAG[i], par_tab$iota[i], par_tab$psi[i], par_tab$zeta[i], par_tab$mu[i], par_tab$nu[i])
    output[[unique_sim_counter]]$unique_sim_number<-unique_sim_counter
    unique_sim_counter <- unique_sim_counter + 1 # Increment the counter for unique simulations
    end_time <- Sys.time()
...
####Save minianalyticdf####
save(minianalyticdf, file= "Manalyticdf_paper1_freq0.RData")
####Save analyticdf####
save(analyticdf, file= "Analyticdf_paper1_freq0.RData")
