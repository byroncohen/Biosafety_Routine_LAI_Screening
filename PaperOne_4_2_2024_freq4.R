...
par_tab <- expand.grid(thetaL= 4/7, sigma= seq(0.5,1,0.1), 
...
for(i in 1:nrow(par_tab)){ #for every row in parameter table representing a unique parameter combination [outer loop]
  ...
  for(j in 1:n_sims){ #for every simulation... [inner loop]
    ...
    unique_sim_number =4*n_sims*nrow(par_tab)+(i - 1) * n_sims + j
...
####Save minianalyticdf####
save(minianalyticdf, file= "Manalyticdf_paper1_freq4.RData")
####Save analyticdf####
save(analyticdf, file= "Analyticdf_paper1_freq4.RData")
